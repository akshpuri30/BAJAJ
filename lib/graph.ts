...
export function checkCycle(
  ...
  for (const next of neighbors) {
    if (!visited.has(next)) {
      if (checkCycle(next, adj, visited, recStack)) return true;
...
export function createTree(root: string, adj: Map<string, string[]>): Record<string, any> {
  const tree: Record<string, any> = {};
  const children = adj.get(root) || [];
  children.sort(); // Sort children alphabetically
  
  for (const child of children) {
    tree[child] = createTree(child, adj);
  }
  return tree;
}
