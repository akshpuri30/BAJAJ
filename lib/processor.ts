...
import { getConnectedComponents, checkCycle, createTree, computeDepth } from './graph';

export function handleInput(rawData: string[]): ApiResponse {
  ...
  for (const comp of components) {
    ...
    for (const node of compNodes) {
      if (!visited.has(node)) {
        if (checkCycle(node, dirAdj, visited, recStack)) {
          hasCycle = true;
          break;
        }
      }
    ...
    if (hasCycle) {
      ...
      tree[root] = createTree(root, dirAdj);
  ...
  trees.sort((a, b) => a.root.localeCompare(b.root));
